    ...
    with outfile_ctx as outfile:
        ...
        if needs_full_pass and not pairing_enabled and not list_files and not tree_view:
            ...
            for i, item in enumerate(all_combined_items):
                ...
                rel_p_str = rel_p.as_posix()
                status = stats.get('file_statuses', {}).get(rel_p_str)
                lang = utils.get_language_tag(file_path, content=processed, overrides=processor.custom_languages)
...
def _print_execution_summary(stats, args, pairing_enabled, destination_desc=None, duration=None, source_desc=None, mirror_enabled=False):
    ...
    if stats.get('top_files'):
        ...
        # Determine which columns to show based on width to preserve path visibility
        show_language = term_width >= 100
        ...
        if show_language: header_parts.append(f"{'LANGUAGE':<11}")
        ...
        for item in top:
            ...
            f_lines = item[4] if len(item) > 4 else 0
            lang = item[5] if len(item) > 5 else "text"
            ...
            # Language tag (responsive)
            if show_language:
                display_lang = _truncate_path(lang, 11)
                row_parts.append(f"{C_BOLD}{display_lang:<11}{C_RESET}")
    ...
    if folder_stats:
        ...
        # Calculate dynamic overhead for path width
        # TOKENS: 13, LINES: 13, SIZE: 13, %: 7, DISTRIBUTION: 13, STATUS: 7, FILES: 8
        overhead = 13 + 7 + 13 + 8 + 4 # SIZE, %, DISTRIBUTION, FILES + indentation
        if has_tokens: overhead += 13
        if has_lines: overhead += 13
        if has_status: overhead += 7
        ...
        # Build dynamic header
        primary_metric = 'tokens' if has_tokens else ('lines' if has_lines else 'size')
        header_parts = []
        if has_tokens:
            header_parts.append(_format_header('TOKENS', 'tokens', primary_metric))
        if has_lines:
    ...
    if files_by_lang:
        ...
        # TOKENS: 12+1, LINES: 12+1, SIZE: 12+1, %: 6+1, DISTRIBUTION: 12+1, STATUS: 6+1, FILES: 7+1, % FILES: 7+1
        overhead = 13 + 7 + 13 + 8 + 8 + 4 # SIZE, %, DISTRIBUTION, FILES, % FILES + indentation
