...
def _convert_to_json_friendly(obj):
    """Recursively convert objects (for example, Path) to JSON-compatible types."""
...
def _render_single_pass(template, replacements):
    ...
    more specific markers (for example, {{DIR_SLUG}}) are preferred over
    shorter ones (for example, {{DIR}}).
    """
...
def _get_file_git_info(file_path, repo_root):
    ...
    try:
        # %an: author name, %ai: author date (ISO 8601-based), %s: subject
...
class FileProcessor:
    ...
    def _backup_file(self, file_path):
        ...
        details (for example, the date it was changed). If ``create_backups`` is ``False``
...
def _print_execution_summary(stats, args, pairing_enabled, destination_desc=None, duration=None, source_desc=None):
    ...
    def _split_unit(s):
        """Split a string for example '1.50 MB' or '1,000 tokens' into (value, unit)."""
        parts = s.split(' ', 1)
