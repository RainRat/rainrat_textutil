...
def find_and_combine_files(
    ...
    with outfile_ctx as outfile:
        ...
        if not pairing_enabled and not list_files and not tree_view:
            # Add global header/footer tokens if token limit pass was skipped
            if not token_limit_pass_performed and (not dry_run or estimate_tokens) and output_format in ('text', 'markdown', 'xml'):
                if global_header:
