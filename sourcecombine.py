...
def _get_sha256_hash(data: str | bytes) -> str:
    """Return the SHA-256 hash of the provided string or bytes."""
...
def main():
    ...
    project_group.add_argument(
        "--project-author",
        metavar="NAME",
        help="Override the project author.",
    )
    project_group.add_argument(
        "--project-description",
        metavar="TEXT",
    ...
    utility_group.add_argument(
        ...
        default=False,
        metavar="QUERY",
        help="Show active text search-and-replace rules (optionally filtered by QUERY) and exit. Use --json for machine-readable output.",
    )
    utility_group.add_argument(
        "--extract",
        action="store_true",
    ...
    utility_group.add_argument(
        ...
        "--clean",
        action="store_true",
        help="Remove all '.bak' files from target folders. Use this to clean up after '--apply-in-place'. Supports --dry-run and --json.",
    )
    utility_group.add_argument(
        "--list-backups",
        "--list-bak",
    ...
    if list_rep_val:
        ...
        if getattr(args, 'replace', None):
            regex_rules = config.setdefault('processing', {}).setdefault('regex_replacements', [])
            for pat, repl in args.replace:
                regex_rules.append({'pattern': pat, 'replacement': repl})

        if getattr(args, 'replace_line', None):
            line_rules = config.setdefault('processing', {}).setdefault('line_regex_replacements', [])
            for pat, repl in args.replace_line:
                line_rules.append({'pattern': pat, 'replacement': repl})
...
def verify_files(sources, root_folder=".", config=None, show_diff=False, repair=False, dry_run=False, strip_components=0, json_format=False, quiet=False):
    ...
    verification_bar = _progress_bar(
        files_to_verify,
        desc="Repairing files" if repair else "Verifying files",
        unit="file",
        enabled=show_progress
    )

    def _write_msg(msg, is_error=False):
        if not json_format and (not quiet or is_error):
            if show_progress and hasattr(verification_bar, "write"):
                verification_bar.write(msg, file=sys.stdout)
            else:
                print(msg)

    def _update_stats(matches_inc=0, mismatches_inc=0, missing_inc=0, repaired_inc=0):
        nonlocal matches, mismatches, missing, repaired
        matches += matches_inc
        mismatches += mismatches_inc
        missing += missing_inc
        repaired += repaired_inc
        if show_progress:
            verification_bar.set_postfix(
                ok=matches,
                mismatch=mismatches,
                missing=missing,
                repaired=repaired
    ...
    try:
        for rel_path_str, expected_content, meta in verification_bar:
                ...
                if repair and expected_content is not None:
                    if dry_run:
                        if json_format:
                            ...
                            _write_msg(f"  {C_CYAN}{'[REPAIR]':<10}{C_RESET}  {rel_path_str} {C_DIM}(would create missing file){C_RESET}")
                        ...
                        try:
                            ...
                            if json_format:
                                ...
                                _write_msg(f"  {C_GREEN}{'[REPAIRED]':<10}{C_RESET}  {rel_path_str} {C_DIM}(created missing file){C_RESET}")
                            _update_stats(repaired_inc=1)
                        except OSError as e:
                            if json_format:
                                json_results.append({"path": rel_path_str, "status": "ERROR", "detail": f"failed to repair: {e}"})
                            else:
                                _write_msg(f"  {C_RED}{'[ERROR]':<10}{C_RESET}  {rel_path_str} {C_DIM}(failed to repair: {e}){C_RESET}")
                            _update_stats(missing_inc=1)
                else:
                    if json_format:
                        json_results.append({"path": rel_path_str, "status": "MISSING", "detail": "missing file"})
                    else:
                        _write_msg(f"  {C_RED}{'[MISSING]':<10}{C_RESET}  {rel_path_str}", is_error=True)
                    _update_stats(missing_inc=1)
                continue
                ...
                try:
                    ...
                    if actual_sha == expected_sha:
                        if json_format:
                            ...
                            _write_msg(f"  {C_GREEN}{'[OK]':<10}{C_RESET}  {rel_path_str} {C_DIM}(hash match){C_RESET}")
                        _update_stats(matches_inc=1)
                    else:
                        if repair and expected_content is not None:
                            if dry_run:
                                if json_format:
                                    ...
                                    _write_msg(f"  {C_CYAN}{'[REPAIR]':<10}{C_RESET}  {rel_path_str} {C_DIM}(would fix hash mismatch){C_RESET}")
                                _update_stats(repaired_inc=1)
                            else:
                                try:
                                    ...
                                    if json_format:
                                        ...
                                        _write_msg(f"  {C_GREEN}{'[REPAIRED]':<10}{C_RESET}  {rel_path_str} {C_DIM}(fixed hash mismatch){C_RESET}")
                                    _update_stats(repaired_inc=1)
                                except OSError as e:
                                    if json_format:
                                        json_results.append({"path": rel_path_str, "status": "ERROR", "detail": f"failed to repair: {e}"})
                                        ...
                                        _write_msg(f"  {C_RED}{'[ERROR]':<10}{C_RESET}  {rel_path_str} {C_DIM}(failed to repair: {e}){C_RESET}")
                                    _update_stats(mismatches_inc=1)
                        else:
                            if json_format:
                                json_results.append({"path": rel_path_str, "status": "MISMATCH", "detail": "hash mismatch"})
                            else:
                                _write_msg(f"  {C_RED}{'[MISMATCH]':<10}{C_RESET}  {rel_path_str} {C_DIM}(hash mismatch){C_RESET}", is_error=True)
                            _update_stats(mismatches_inc=1)
                    ...
                    if json_format:
                        ...
                        _write_msg(f"  {C_RED}{'[ERROR]':<10}{C_RESET}  {rel_path_str} {C_DIM}({e}){C_RESET}")
                    _update_stats(mismatches_inc=1)
                continue
            ...
            if expected_content is not None:
                ...
                if actual_content.replace('\r\n', '\n') == expected_content.replace('\r\n', '\n'):
                    if json_format:
                        ...
                        _write_msg(f"  {C_GREEN}{'[OK]':<10}{C_RESET}  {rel_path_str} {C_DIM}(content match){C_RESET}")
                    _update_stats(matches_inc=1)
                else:
                    if repair:
                        if dry_run:
                            if json_format:
                                ...
                                _write_msg(f"  {C_CYAN}{'[REPAIR]':<10}{C_RESET}  {rel_path_str} {C_DIM}(would fix content mismatch){C_RESET}")
                            _update_stats(repaired_inc=1)
                        else:
                            try:
                                _create_backup_if_enabled(target_path, create_backups)
                                ...
                                if json_format:
                                    ...
                                    _write_msg(f"  {C_GREEN}{'[REPAIRED]':<10}{C_RESET}  {rel_path_str} {C_DIM}(fixed content mismatch){C_RESET}")
                                _update_stats(repaired_inc=1)
                            except OSError as e:
                                if json_format:
                                    json_results.append({"path": rel_path_str, "status": "ERROR", "detail": f"failed to repair: {e}"})
                        ...
                        if json_format:
                            ...
                            _write_msg(f"  {C_RED}{'[MISMATCH]':<10}{C_RESET}  {rel_path_str} {C_DIM}(content mismatch){C_RESET}", is_error=True)
                        _update_stats(mismatches_inc=1)
                        if show_diff:
                            _print_diff(actual_content, expected_content, rel_path_str)
                continue
            ...
            if json_format:
                ...
                _write_msg(f"  {C_YELLOW}{'[SKIPPED]':<10}{C_RESET}  {rel_path_str} {C_DIM}(no hash or content to verify against){C_RESET}")
    finally:
        verification_bar.close()
    ...
    if json_format:
        ...
        print(f"{C_BOLD}{C_CYAN}{'=' * 27}{C_RESET}\n")
    ...
    for target in targets:
        ...
        for backup_path in backup_files:
            ...
            if dry_run:
                ...
                results.append({
                    "path": rel_path_str,
                    "backup_path": _get_rel_path(backup_path, root_path).as_posix(),
...
def print_ignore_patterns(query=None, json_format=False, config=None):
    """Print active ignore patterns from ignore files and configuration exclusions, optionally filtered by a query."""
    if config is None:
        config = copy.deepcopy(utils.DEFAULT_CONFIG)
        utils.validate_config(config)
    else:
    ...
    total_available = sum(len(pats) for pats in categories.values())
    total_matched = 0
    ...
    if json_format:
        if query:
            ...
            for cat, pats in categories.items():
                matched = [p for p in pats if query_lower in p.lower()]
                if matched:
                    filtered_cats[cat] = matched
    ...
    for category, patterns in categories.items():
        if query_lower:
            filtered_pats = [p for p in patterns if query_lower in p.lower()]
        else:
            filtered_pats = patterns
...
def print_replacements(query=None, json_format=False, config=None):
    """Print active text and line replacement rules, optionally filtered by a query."""
    if config is None:
        config = copy.deepcopy(utils.DEFAULT_CONFIG)
        utils.validate_config(config)
    else:
    ...
    proc_opts = config.get('processing', {}) or {}
    regex_rules = list(proc_opts.get('regex_replacements') or [])
    line_rules = list(proc_opts.get('line_regex_replacements') or [])

    # Filter rules if query is specified
    if query:
        query_lower = query.lower()
        filtered_regex = [
            r for r in regex_rules
        ...
        query_lower = None
        filtered_regex = regex_rules
        filtered_line = line_rules

    total_available = len(regex_rules) + len(line_rules)
    total_matched = len(filtered_regex) + len(filtered_line)

    if json_format:
        output = {
            "regex_replacements": filtered_regex,
            "line_regex_replacements": filtered_line,
            "total": total_matched if query_lower else total_available
        }
        print(json.dumps(output, indent=2))
        return

    title_suffix = f" (FILTERED BY '{query}')" if query_lower else ""
    print(f"\n{C_BOLD}{C_CYAN}=== ACTIVE REPLACEMENT RULES{title_suffix} ==={C_RESET}")

    if total_matched == 0:
        if query_lower:
            print(f"\n  {C_YELLOW}No replacement rules matched the filter query '{query}'.{C_RESET}")
        else:
            print(f"\n  {C_DIM}No replacement rules are currently configured.{C_RESET}")
    else:
        if filtered_regex:
            print(f"\n  {C_BOLD}Text Replacement Rules (Config / CLI):{C_RESET}")
            for rule in filtered_regex:
                pat = rule.get('pattern', '')
                repl = rule.get('replacement', '')
                print(f"    {C_BOLD}{C_CYAN}'{pat}'{C_RESET} -> {C_GREEN}'{repl}'{C_RESET}")

        if filtered_line:
            print(f"\n  {C_BOLD}Line Replacement Rules (Config / CLI):{C_RESET}")
    ...
    count_label = f"Matching: {total_matched}" if query_lower else f"Total: {total_available}"
    print(f"\n  {C_BOLD}{count_label}{C_RESET} active replacement rules configured.")
    print(f"\n{C_BOLD}{'=' * 40}{C_RESET}\n")
...
def _print_execution_summary(stats, args, pairing_enabled, destination_desc=None, duration=None, source_desc=None, mirror_enabled=False):
    ...
    found_value_style = f"{C_BOLD}{C_CYAN}"
    ...
    if has_any_skips:
        ...
        print(f"    {C_DIM}├── {'Included:':<{label_width - 4}}{C_RESET}{C_BOLD}{C_GREEN}{total_included:12,}{C_RESET}{C_DIM}{included_unit:<8}{C_RESET} {C_DIM}({C_RESET}{C_BOLD}{C_CYAN}{included_percent:>6.1f}{C_RESET}{C_DIM}%){C_RESET}", file=sys.stderr)

        if has_skipped_files:
            skipped_connector = "├── " if has_skipped_folders else "└── "
            skipped_unit = f" {_plural(total_filtered, 'file')}"
            print(f"    {C_DIM}{skipped_connector}{'Skipped:':<{label_width - 4}}{C_RESET}{C_BOLD}{C_YELLOW}{total_filtered:12,}{C_RESET}{C_DIM}{skipped_unit:<8}{C_RESET} {C_DIM}({C_RESET}{C_BOLD}{C_CYAN}{skipped_percent:>6.1f}{C_RESET}{C_DIM}%){C_RESET}", file=sys.stderr)
            ...
            if relevant_reasons:
                ...
                for i, (reason, count) in enumerate(sorted_reasons):
                    ...
                    print(f"    {C_DIM}{outer_skipped_indent}{connector}{display_reason:<{label_width - 8}}{C_RESET}{C_BOLD}{C_CYAN}{count:12,}{C_RESET}{C_DIM}{reason_unit:<8}{C_RESET} {C_DIM}({C_RESET}{C_BOLD}{C_CYAN}{reason_percent:>6.1f}{C_RESET}{C_DIM}%){C_RESET}", file=sys.stderr)

        if has_skipped_folders:
            folder_unit = f" {_plural(excluded_folders, 'folder')}"
            print(f"    {C_DIM}└── {'Skipped Folders:':<{label_width - 4}}{C_RESET}{C_BOLD}{C_CYAN}{excluded_folders:12,}{C_RESET}{C_DIM}{folder_unit:<8}{C_RESET}", file=sys.stderr)
