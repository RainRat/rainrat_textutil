...
*   `--export-ignore` (`--export-ig`): Export all active ignore patterns (from loaded ignore files and configuration exclusions) to a `.sourcecombineignore` file (or custom path) and exit. Use `-` to output to standard output (`stdout`), or `--json` for machine-readable output.
