...
*   `--extension` (`--ext`): Include only files with specific extensions (for example, `py`, `.js`).
*   `--exclude-extension` (`--exclude-ext`): Skip files with specific extensions (for example, `log`, `.tmp`).
