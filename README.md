...
*   **File Extraction:** Rebuild your original files and folders from combined files (like JSON, XML, JSONL, or Markdown). If you do not provide an input, the tool automatically looks for `combined_files.txt` (or `.md`, `.json`, `.xml`, `.jsonl`). Filtering, sorting, and processing rules are supported. Batch process multiple files or entire folders.
...
*   `--extract`: Rebuild your original files and folders from combined files (like JSON, XML, JSONL, or Markdown). You can read from one or more files, folders, your terminal (`-`), or the system clipboard. If you do not provide any input, the tool automatically searches for `combined_files.txt`, `.md`, `.json`, `.xml`, or `.jsonl` in your current folder. For example: `python sourcecombine.py --extract outputs/`. Filtering, sorting, and preview options are supported. Line numbers are removed automatically unless you use `--keep-line-numbers`.
