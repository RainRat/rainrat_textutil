...
*   `--export-ignore [PATH]` (`--export-ig`): Export all active ignore patterns (from loaded ignore files and configuration exclusions) to a `.sourcecombineignore` file or custom `PATH` and exit. Use `-` to print to standard output (`stdout`). Supports `--json` machine-readable format.
