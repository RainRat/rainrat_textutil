...
def test_get_project_identity_package_json_invalid(tmp_path):
...
def test_get_project_identity_pyproject_toml_section_match(tmp_path):
...
def test_get_project_identity_pyproject_toml_invalid_regex(tmp_path):
    pyproject = tmp_path / "pyproject.toml"
    # Create a file that exists but has no name match
    pyproject.write_text("[project]\nfoo = 'bar'", encoding='utf-8')
    assert utils.get_project_identity(tmp_path)["project_name"] == tmp_path.name

def test_get_project_identity_exception_fallback(tmp_path):
...
def test_get_project_identity_pyproject_exception_handled(tmp_path):
    pyproject = tmp_path / "pyproject.toml"
