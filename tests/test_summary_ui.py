...
def test_summary_terminal_size_fallback(capsys):
    ...
    captured = capsys.readouterr()
    assert "File Types (by size)" in captured.err
...
def test_file_types_redesign_sorting_and_others(monkeypatch, capsys):
    ...
    ext_lines = [line for line in stderr.splitlines() if "DISTRIBUTION" not in line and "[" in line and "]" in line and "EXTENSION" not in line and "===" not in line]
