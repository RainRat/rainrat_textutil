...
def test_summary_printing(monkeypatch, capsys):
    ...
    stats = {
        ...
        'files_by_language': {
            'python': 10, 'text': 5, 'markdown': 3, 'c': 1, 'cpp': 1,
            'java': 1, 'javascript': 1, 'typescript': 1, 'css': 1, 'html': 1,
            'json': 1, 'xml': 1, 'yaml': 1
        },
        'total_tokens': 5000,
...
def test_summary_throughput_line(capsys):
    ...
    assert "files/s" in captured.err
    assert "500.00" in captured.err
    assert "B/s" in captured.err
...
def test_file_types_redesign_sorting_and_others(monkeypatch, capsys):
    ...
    ext_lines = [line for line in stderr.splitlines() if " [" in line and "]" in line and ("python" in line or "text" in line or "(others)" in line) and "LANGUAGE" not in line and "===" not in line and "tokens" not in line]

    assert "python" in ext_lines[0]
    assert "text" in ext_lines[1]

    # Check "others" aggregation (12 languages total, top 10 shown, others aggregated)
