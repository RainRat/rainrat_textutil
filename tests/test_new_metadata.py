...
def test_get_project_identity_package_json(tmp_path):
...
def test_get_project_identity_pyproject_toml_top(tmp_path):
...
def test_get_project_identity_pyproject_toml_project_section(tmp_path):
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('[project]\nname = "test-section"', encoding='utf-8')
    assert utils.get_project_identity(tmp_path)["project_name"] == "test-section"

def test_get_project_identity_fallback_folder(tmp_path):
