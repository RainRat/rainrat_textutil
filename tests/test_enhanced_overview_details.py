...
def test_enhanced_overview_text():
    ...
    assert "1 files ( 50.0%) •        0 lines •  80.0% [########--]" in overview
    assert ".md" in overview
    assert "1 files ( 50.0%) •        0 lines •  20.0% [##--------]" in overview
...
def test_enhanced_overview_markdown():
    ...
    assert "| `src/main.py` | 400 | 0 | 1.46 KB | 80.0% |" in overview

    # Check File Types table
    assert "## File Types" in overview
    assert "| Extension | Count | % Files | Lines | % Tokens |" in overview
    assert "| `.py` | 1 | 50.0% | 0 | 80.0% |" in overview
    assert "| `.md` | 1 | 50.0% | 0 | 20.0% |" in overview
